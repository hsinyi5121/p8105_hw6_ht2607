...
beta_plot
```
since beta2 is negative, which we cannot take the log to the negative value; therefore, 
the when ploting there are 3361 data out of 5000 from the bootstraping result removed. 
beta_plot
Warning message:
Removed 3361 rows containing non-finite values (`stat_bin()`). 
...
Setting Seed: set.seed(1) ensures reproducibility by fixing the seed value for any random number generation.

Cross-Validation: crossv_mc is a function used to create a cross-validation scheme. It appears to be splitting the birthweight_df dataset into multiple folds for cross-validation, with 100 repetitions (n = 100) and a 80:20 train-test split (test = 0.2).

Data Preparation: The resulting birthweight_dataset is processed to convert the 'train' and 'test' splits into tibbles (rectangular data frames) using map function from the purrr package.
...
Evaluation: The code then evaluates the performance of these models on the test sets using root mean square error (rmse). This is done by employing the map2_dbl function from the purrr package to calculate the RMSE for each model applied to the respective test set.

Creation of Metrics: Finally, the code creates new columns in birth_df (rmse_my_model, rmse_model_1, rmse_model_2) to store the computed RMSE values for the respective models.
