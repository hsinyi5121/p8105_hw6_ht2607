...
violin plot for 3 models (one from the previous and other from the model
1 and 2)

``` r
# Assuming birthweight_df, birthweight_model, model_1, and model_2 are defined
...
# Gather predictions for Model 1 and Model 2
model_1_predictions <- predict(model_1, newdata = birthweight_df)
model_2_predictions <- predict(model_2, newdata = birthweight_df)

# Simulate predictions for the training model
set.seed(1)
training_predictions <- predict(birthweight_model)

# Combine predictions with respective models and data
predictions_df <- data.frame(
  Model = rep(c("Training Model", "Model 1", "Model 2"), each = nrow(birthweight_df)),
  Prediction = c(training_predictions, model_1_predictions, model_2_predictions),
  stringsAsFactors = FALSE
)

# Create a violin plot
ggplot(predictions_df, aes(x = Model, y = Prediction, fill = Model)) +
  geom_violin() +
  labs(x = "Model", y = "Predicted Values", title = "Comparison of Predicted Values") +
  theme_minimal()
```
